...
class LunchTime(http.Controller):
    ...
    def lunch_time_submit(self, **post):
        ...
        if attendance:
            if attendance.x_studio_hora_de_comida:
                attendance.sudo().write({'x_studio_regreso_de_comida': current_time})
                message = 'Su hora de regreso ha sido registrada'
            else:
                attendance.sudo().write({'x_studio_hora_de_comida': current_time})
